...
//                    'id' => $param['id'],
                ...
                $flag = $insulatingequipment->insertInsulatingequipment($data);
...
//                    'date' => date("Y-m-d H:i:s"),//添加时间
     ...
     * 删除一条绝缘工器具信息
     */
    public function insulatingequipmentDel()
    {
        $insulatingequipment = new InsulatingequipmentModel();
        if(request()->isAjax()) {
            $param = input('post.');
            ...
            $flag = $insulatingequipment->delInsulatingequipment($param['id']);
            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }
    }
    ...
    public function getversion()
    {
        if(request()->isAjax()){
            ...
            $selfid = $param['selfid'];
            $insulatingequipment= new InsulatingequipmentModel();
            $data = $insulatingequipment->getVersion($selfid);
            return json(['code'=> 1, 'data' => $data]);
        }
        return $this->fetch();
    }
    ...
    public function importExcel()
    {
        ...
        $info = $file->move(ROOT_PATH . 'public' . DS . 'uploads/safety/import/insulatingequipment');
        if($info){
            // 调用插件PHPExcel把excel文件导入数据库
            Loader::import('PHPExcel\Classes\PHPExcel', EXTEND_PATH);
            $exclePath = $info->getSaveName();  //获取文件名
            $file_name = ROOT_PATH . 'public' . DS . 'uploads/safety/import/insulatingequipment' . DS . $exclePath;   //上传文件的地址
...
//            return json(['1'=> $number_index, '2' => $equip_name_index,'3' =>$model_index,'4' => $equip_num_index,'5' => $manufactur_unit_index,'6' => $date_production_index,'7' => $current_state_index,'8' => $safety_inspection_num_index,'9' => $inspection_unit_index,'10' => $entry_time_index,'11' => $equip_state_index,'12' => $remark_index]);
            ...
            $success = Db::name('safety_insulating_equipment')->insertAll($insertData);
            if($success !== false){
    ...
    public function getcount()
    {
        ...
        if(request()->isAjax()) {
            ...
            $flag = $insulatingequipment->getallcount($where);
    ...
    public function exportExcel()
    {
        ...
        $list = $insulatingequipment->getList($idArr);
