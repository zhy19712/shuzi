...
//                    'selfid' => $param['selfid'],//区别类别
                ...
                $flag = $insulating->insertInsulatinginner($data);
...
//        }
        ...
        $info = $file->move(ROOT_PATH . 'public' . DS . 'uploads/safety/import/insulatinginner');
        if($info){
            // 调用插件PHPExcel把excel文件导入数据库
            Loader::import('PHPExcel\Classes\PHPExcel', EXTEND_PATH);
            $exclePath = $info->getSaveName();  //获取文件名
            $file_name = ROOT_PATH . 'public' . DS . 'uploads/safety/import/insulatinginner' . DS . $exclePath;   //上传文件的地址
...
//                    $insertData[$k]['selfid'] = $selfid;
            ...
            $success = Db::name('safety_insulating_equipment_inner')->insertAll($insertData);
            if($success !== false){
