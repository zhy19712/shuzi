...
use app\safety\model\PersonalinnerModel;
use think\Db;
use think\Loader;

class Personalinner extends Base
{
    /*
     * 获取一条个人防护装备信息
     */
    ...
    {
        if(request()->isAjax()){
            $personal = new PersonalinnerModel();
            $param = input('post.');
...
//                    'selfid' => $param['selfid'],//区别类别
                ...
                $flag = $personal->insertPersonalinner($data);
...
//                    'date' => date("Y-m-d H:i:s"),//添加时间
                ...
                $flag = $personal->editPersonalinner($data);
                return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
            }
    ...
    public function personalDel()
    {
        ...
        if(request()->isAjax()) {
            ...
            $flag = $personal->delPersonalinner($param['id']);
            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }
    }
...
//        }
        ...
        $info = $file->move(ROOT_PATH . 'public' . DS . 'uploads/safety/import/personalinner');
        if($info){
            // 调用插件PHPExcel把excel文件导入数据库
            Loader::import('PHPExcel\Classes\PHPExcel', EXTEND_PATH);
            $exclePath = $info->getSaveName();  //获取文件名
            $file_name = ROOT_PATH . 'public' . DS . 'uploads/safety/import/personalinner' . DS . $exclePath;   //上传文件的地址
...
//                    $insertData[$k]['selfid'] = $selfid;
            ...
            $success = Db::name('safety_personal_equipment_inner')->insertAll($insertData);
            if($success !== false){
    ...
    public function exportExcel()
    {
        ...
        if($idArr['0'] == "all")
        {
            $idArr = $personal ->getallid();
        }
        $name = '个人防护装备'.date('Y-m-d H:i:s'); // 导出的文件名

        $list = $personal->getList($idArr);
