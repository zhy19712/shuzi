...
class UnitEngineering extends Base
{
    ...
    public function editStandardDeviation()
    {
        if(request()->isAjax()){
            $param = input('post.');
            // 前台需要传递的参数 genre 1支护2锚杆3混凝土 gid 支护,锚杆,混凝土主键  type 1喷砼强度 2锚杆砂浆强度
            // 前台需要传递的参数 construct_value 施工单位试验强度值(是数组) supervise_value 监理单位试验强度值 standard_value 设计强度标准值
            ...
            $rule = [
                ...
                ['construct_value', 'require', '请填写施工单位试验强度值'],
                ['supervise_value', 'require', '请填写监理单位试验强度值'],
                ['standard_value', 'require', '请填写设计强度标准值'],
            ];
            ...
            foreach ($construct_arr as $v){
                $data[$k]['genre'] = $genre;
                $data[$k]['gid'] = $gid;
                $data[$k]['unit_type'] = 1; // unit_type 1施工单位2监理单位 根据前台传递的数组名称判断是哪个单位的
                $data[$k]['type'] = $type;
                $data[$k]['intensity_value'] = $v;
                $data[$k]['standard_value'] = $standard_value;
                $k++;
            }
            foreach ($supervise_arr as $v){
                $data[$k]['genre'] = $genre;
                $data[$k]['gid'] = $gid;
                $data[$k]['unit_type'] = 2; // unit_type 1施工单位2监理单位 根据前台传递的数组名称判断是哪个单位的
                $data[$k]['type'] = $type;
                $data[$k]['intensity_value'] = $v;
                $data[$k]['standard_value'] = $standard_value;
                $k++;
    ...
    public function standardDeviation()
    {
        if(request()->isAjax()){
            ...
            return json(['code' => 1, 'data' => $flag, 'msg' => '强度值']);
        }
    }
